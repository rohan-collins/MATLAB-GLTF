    function accessor_idx=addBinaryData(obj,data,componentType,dataCount,minmax,target)
    % Add binary data to the model.
    %
    % ADDBINARYDATA(OBJ,DATA,COMPONENTTYPE,DATACOUNT,MINMAX) adds binary
    ...
    gltf.GLTF.validateString(componentType,componentType_str);
    dataCount=upper(dataCount);
    gltf.GLTF.validateString(dataCount,dataCount_str);
    if(nargin<6)
        target=[];
    else
        gltf.GLTF.validateString(target,target_str);
    ...
    accessor_idx=numel(obj.accessors);
    bufferViewCount=numel(obj.bufferViews);
    bufferCount=numel(obj.buffers);
    if(bufferCount==0)
        obj.buffers{1}=[];
        bufferOffset=0;
        bufferCount=1;
    else
        bufferOffset=numel(obj.buffers{bufferCount});
    end
    obj.accessors{accessor_idx+1}.bufferView=uint32(bufferViewCount);
    obj.accessors{accessor_idx+1}.componentType=component;
    if(or(or(dataCount=="MAT2",dataCount=="MAT3"),dataCount=="MAT4"))
        data=reshape(data,size(data,1)*size(data,1),size(data,3))';
    end
    if(dataCount=="SCALAR")
        obj.accessors{accessor_idx+1}.count=uint32(numel(data));
    else
        obj.accessors{accessor_idx+1}.count=uint32(size(data,1));
    end
    obj.accessors{accessor_idx+1}.type=dataCount;
    if(minmax)
        if(dataCount=="SCALAR")
            obj.accessors{accessor_idx+1}.max=num2cell(max(data(:)));
            obj.accessors{accessor_idx+1}.min=num2cell(min(data(:)));
        elseif(ismember(dataCount,["VEC2","VEC3","VEC4"]))
            obj.accessors{accessor_idx+1}.max=max(data,[],1)';
            obj.accessors{accessor_idx+1}.min=min(data,[],1)';
        end
    end
    obj.bufferViews{bufferViewCount+1}.buffer=uint32(bufferCount-1);
    obj.bufferViews{bufferViewCount+1}.byteLength=uint32(numel(data)*byteCount);
    obj.bufferViews{bufferViewCount+1}.byteOffset=uint32(bufferOffset);
    if(~isempty(target))
        obj.bufferViews{bufferViewCount+1}.target=uint32(target);
    end
    binData=typecast(castFcn(reshape(data',1,[])),'uint8');
    obj.buffers{bufferCount}=[obj.buffers{bufferCount};binData(:)];
end
