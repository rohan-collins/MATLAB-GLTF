function skin_idx=addSkin(obj,joints,varargin)
    % Add a skin for skinning animation.
    %
    % ADDSKIN(OBJ,JOINTS) adds a skin with specified joints to GLTF and
    % returns its index.
    %
    ...
    if(~isempty(inverseBindMatrices))
        skin.inverseBindMatrices=obj.addBinaryData(inverseBindMatrices,"FLOAT","MAT4",false);
    end
    if(~isprop(obj,'skins'))
        obj.addprop('skins');
    end
    skin_idx=numel(obj.skins);
    obj.skins=[obj.skins;{skin}];
end
