function sampler_idx=addTextureSampler(obj,varargin)
    % Add a texture sampler.
    %
    % ADDTEXTURESAMPLER(OBJ) adds a new empty texture sampler to GLTF and
    % returns its index.
    %
    ...
    if(~isprop(obj,'samplers'))
        obj.addprop('samplers');
    end
    sampler_idx=numel(obj.samplers);
    obj.samplers=[obj.samplers;sampler];
end
