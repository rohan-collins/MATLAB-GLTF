function addAnimation(obj,samplers,channels,varargin)
    % Add an animation to the mode.
    %
    % ADDANIMATION(OBJ,SAMPLERS,CHANNELS) adds an animation to GLTF, where
    ...
    if(~isprop(obj,'animations'))
        obj.addprop('animations');
    end
    animationstruct={struct('samplers',[],'channels',[])};
    animationstruct{1}.samplers=gltf.GLTF.toCells(samplers);
    animationstruct{1}.channels=gltf.GLTF.toCells(channels);
    if(~ismissing(name))
        animationstruct{1}.name=name;
    end
    if(~isempty(events))
        animationstruct{1}.extensions.MSFT_audio_emitter.events=gltf.GLTF.toCells(events);
    end
    if(isempty(obj.animations))
        obj.animations=animationstruct;
    else
        obj.animations=[obj.animations(:);animationstruct(:)]';
    end
end
