function camera_idx=addPerspectiveCamera(obj,varargin)
    % Add a perspective camera.
    %
    % ADDPERSPECTIVECAMERA(OBJ) adds a perspective camera with the default
    ...
    if(~isprop(obj,'cameras'))
        obj.addprop('cameras');
    end
    camera_idx=numel(obj.cameras);
    obj.cameras=[obj.cameras;{camerastruct}];
end
