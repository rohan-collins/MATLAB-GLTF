function image_idx=addImage(obj,image,varargin)
    % Add an image.
    %
    % ADDIMAGE(OBJ,FILENAME) adds the image specified by FILENAME to GLTF
    ...
    if(embedTexture)
        image={struct('uri',gltf.GLTF.string2URI(image))};
    else
