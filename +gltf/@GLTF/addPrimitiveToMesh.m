function primitive_idx=addPrimitiveToMesh(obj,mesh_idx,V,varargin)
    % Add a mesh primitive and returns the index.
    %
    % ADDPRIMITIVETOMESH(OBJ,MESH_IDX,V) adds a primitive to GLTF to the
    ...
    ips.addParameter('mode',[],@(x)gltf.GLTF.validateString(x,mode_str_values));
    ...
    if(and(size(indices,2)>3,triangulate))
        indices=gltf.GLTF.toTriangles(indices);
    end
    if(size(indices,2)<3)
        skipTest=true;
    end
    if(and(flatShading,~isempty(indices)))
        ...
        Nv2=gltf.GLTF.vertexNormals(F2,V2);
        VN2=[V2 Nv2];
    ...
    if(~skipTest)
        if(isempty(indices))
            testNormals=gltf.GLTF.vertexNormals(reshape(1:size(V,1),3,[])',V);
            V=V(~any(isnan(testNormals),2),:);
        else
            testNormals=gltf.GLTF.faceNormals(indices,V);
            indices=indices(~any(isnan(testNormals),2),:);
            [~,indices,idx]=gltf.GLTF.vertexNormals(indices,V);
            V=V(idx,:);
    ...
    position=obj.addBinaryData(V,"FLOAT","VEC3",true,"ARRAY_BUFFER");
    attributes=struct('POSITION',position);
    if(normals)
        if(isempty(N))
            if(isempty(indices))
                N=gltf.GLTF.vertexNormals(reshape(1:size(V,1),3,[])',V);
            else
                N=gltf.GLTF.vertexNormals(indices,V);
            end
        end
        attributes.NORMAL=obj.addBinaryData(N,"FLOAT","VEC3",true,"ARRAY_BUFFER");
        if(tangents)
            if(isempty(T))
                texcoord_temp=texcoord(:,:,1);
                if(isempty(indices))
                    T=gltf.GLTF.vertexTangents(reshape(1:size(V,1),3,[])',V,texcoord_temp);
                else
                    T=gltf.GLTF.vertexTangents(indices,V,texcoord_temp);
                end
            else
                if(size(T,2)==3)
                    T=[T ones(size(T,1),1)];
            ...
            attributes.TANGENT=obj.addBinaryData(T,"FLOAT","VEC4",true,"ARRAY_BUFFER");
        end
    end
    primitivestruct=struct('attributes',attributes);
    if(~isempty(indices))
        indices=obj.addBinaryData(reshape(indices',[],1)-1,"UNSIGNED_INT","SCALAR",false,"ELEMENT_ARRAY_BUFFER");
    ...
    if(~isempty(texcoord))
        for i=1:size(texcoord,3)
            primitivestruct.attributes.("TEXCOORD_"+string(i-1))=obj.addBinaryData(texcoord(:,:,i),"FLOAT","VEC2",true,"ARRAY_BUFFER");
        end
    end
    if(~isempty(color))
        if(size(color,2)==3)
            for i=1:size(color,3)
                primitivestruct.attributes.("COLOR_"+string(i-1))=obj.addBinaryData(color(:,:,i),"FLOAT","VEC3",true,"ARRAY_BUFFER");
            end
        elseif(size(color,2)==4)
            for i=1:size(color,3)
                primitivestruct.attributes.("COLOR_"+string(i-1))=obj.addBinaryData(color(:,:,i),"FLOAT","VEC4",true,"ARRAY_BUFFER");
            end
        end
    end
    if(~isempty(joints))
        for i=1:size(joints,3)
            primitivestruct.attributes.("JOINTS_"+string(i-1))=obj.addBinaryData(joints(:,:,i),"UNSIGNED_SHORT","VEC4",true,"ARRAY_BUFFER");
        end
    end
    if(~isempty(weights))
        for i=1:size(weights,3)
            primitivestruct.attributes.("WEIGHTS_"+string(i-1))=obj.addBinaryData(weights(:,:,i),"FLOAT","VEC4",true,"ARRAY_BUFFER");
        end
    end
    if(~isempty(variants))
        obj.addExtension("KHR_materials_variants");
        ...
        for var_idx=1:numel(variants)
            ...
            KHR_materials_variants_struct.mappings(var_idx).variants=gltf.GLTF.toCells(variants(var_idx).variants);
        end
        if(~isprop(obj,'extensions'))
            addprop(obj,'extensions');
        end
        if(isfield(obj.extensions,'KHR_materials_variants') && isfield(obj.extensions.KHR_materials_variants,'variants'))
            start_idx=numel(obj.extensions.KHR_materials_variants.variants)-1;
            obj.extensions.KHR_materials_variants.variants=[obj.extensions.KHR_materials_variants.variants repmat(struct(),1,max_var_idx-start_idx)];
        else
            obj.extensions.KHR_materials_variants.variants=repmat(struct(),1,max_var_idx+1);
        end
        primitivestruct.primitives{1}.extensions.KHR_materials_variants=KHR_materials_variants_struct;
    end
    primitive_idx=numel(obj.meshes{mesh_idx+1}.primitives);
    obj.meshes{mesh_idx+1}.primitives=[obj.meshes{mesh_idx+1}.primitives primitivestruct];
end
