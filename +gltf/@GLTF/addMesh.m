function mesh_idx=addMesh(obj,V,varargin)
    % Add a mesh.
    %
    % ADDMESH(OBJ,V) adds a mesh primitive to GLTF with vertices specified
    ...
    if(normals)
        if(isempty(N))
            if(isempty(indices))
                ...
                [N,indices,idx]=gltf.GLTF.vertexNormals(indices,V);
                V=V(idx,:);
            end
            if(and(tangents,~isempty(T)))
                T=T(idx,:);
    ...
    if(~isempty(texcoord))
        for i=1:size(texcoord,3)
            meshstruct.primitives{1}.attributes.("TEXCOORD_"+string(i-1))=obj.addBinaryData(texcoord(:,:,i),"FLOAT","VEC2",true,"ARRAY_BUFFER");
        end
    end
    if(~isempty(color))
        if(size(color,2)==3)
            for i=1:size(color,3)
                meshstruct.primitives{1}.attributes.("COLOR_"+string(i-1))=obj.addBinaryData(color(:,:,i),"FLOAT","VEC3",true,"ARRAY_BUFFER");
            end
        elseif(size(color,2)==4)
            for i=1:size(color,3)
                meshstruct.primitives{1}.attributes.("COLOR_"+string(i-1))=obj.addBinaryData(color(:,:,i),"FLOAT","VEC4",true,"ARRAY_BUFFER");
            end
        end
    end
    if(~isempty(joints))
        for i=1:size(joints,3)
            meshstruct.primitives{1}.attributes.("JOINTS_"+string(i-1))=obj.addBinaryData(joints(:,:,i),"UNSIGNED_SHORT","VEC4",true,"ARRAY_BUFFER");
        end
    end
    if(~isempty(weights))
        for i=1:size(weights,3)
            meshstruct.primitives{1}.attributes.("WEIGHTS_"+string(i-1))=obj.addBinaryData(weights(:,:,i),"FLOAT","VEC4",true,"ARRAY_BUFFER");
        end
    end
    if(~isempty(variants))
    ...
    mesh_idx=numel(obj.meshes);
    obj.meshes=[obj.meshes meshstruct];
end
