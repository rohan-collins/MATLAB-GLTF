function node_idx=addNode(obj,varargin)
    % Add a node.
    %
    % ADDNODE(OBJ) adds a new empty node to GLTF and returns its index.
    %
    ...
    if(~isempty(children))
        obj.nodes{node_idx+1}.children=gltf.GLTF.toCells(children);
        added=true;
    end
    if(~ismissing(name))
        obj.nodes{node_idx+1}.name=name;
        added=true;
    end
    if(~isempty(mesh))
        obj.nodes{node_idx+1}.mesh=mesh;
        added=true;
    end
    if(~isempty(light))
        obj.nodes{node_idx+1}.extensions=struct('KHR_lights_punctual',struct('light',light));
        added=true;
    end
    if(~isempty(camera))
        obj.nodes{node_idx+1}.camera=camera;
        added=true;
    end
    if(~isempty(translation))
        obj.nodes{node_idx+1}.translation=translation;
        added=true;
    end
    if(~isempty(rotation))
        obj.nodes{node_idx+1}.rotation=rotation;
        added=true;
    end
    if(~isempty(scale))
        obj.nodes{node_idx+1}.scale=scale;
        added=true;
    end
    if(~isempty(skin))
        obj.nodes{node_idx+1}.skin=skin;
        added=true;
    end
    if(and(~isempty(matrix),and(and(isempty(translation),isempty(rotation)),isempty(scale))))
        obj.nodes{node_idx+1}.matrix=matrix(:);
        added=true;
    end
    if(~isempty(weights))
        obj.nodes{node_idx+1}.weights=weights;
        added=true;
    end
    if(or(or(~isempty(instancingTranslation),~isempty(instancingRotation)),~isempty(instancingScale)))
        obj.addExtension("EXT_mesh_gpu_instancing",true);
        if(~isempty(instancingTranslation))
            obj.nodes{node_idx+1}.extensions.EXT_mesh_gpu_instancing.attributes.TRANSLATION=obj.addBinaryData(instancingTranslation,"FLOAT","VEC3",true,"ARRAY_BUFFER");
        end
        if(~isempty(instancingRotation))
            obj.nodes{node_idx+1}.extensions.EXT_mesh_gpu_instancing.attributes.ROTATION=obj.addBinaryData(instancingRotation,"FLOAT","VEC4",true,"ARRAY_BUFFER");
        end
        if(~isempty(instancingScale))
            obj.nodes{node_idx+1}.extensions.EXT_mesh_gpu_instancing.attributes.SCALE=obj.addBinaryData(instancingScale,"FLOAT","VEC3",true,"ARRAY_BUFFER");
        end
        added=true;
    end
    if(~added)
        obj.nodes{node_idx+1}=struct;
    end
    if(addToScene)
        if(isempty(obj.scenes{1}.nodes))
            obj.scenes{1}.nodes=num2cell(node_idx);
        else
            obj.scenes{1}.nodes=[obj.scenes{1}.nodes(:);num2cell(node_idx)]';
        end
    end
end
