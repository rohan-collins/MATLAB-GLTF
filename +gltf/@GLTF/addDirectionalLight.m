function light_idx=addDirectionalLight(obj,varargin)
    % Add a directional light source.
    %
    % ADDDIRECTIONALLIGHT(OBJ) adds a white directional light source with
    ...
    obj.addExtension("KHR_lights_punctual");
    if(~isprop(obj,'extensions'))
        obj.addprop('extensions');
    end
    if(isfield(obj.extensions,'KHR_lights_punctual'))
        light_idx=numel(obj.extensions.KHR_lights_punctual.lights);
        obj.extensions.KHR_lights_punctual.lights=[obj.extensions.KHR_lights_punctual.lights {lightstruct}];
    else
        obj.extensions.KHR_lights_punctual.lights={lightstruct};
        light_idx=0;
    end
end
