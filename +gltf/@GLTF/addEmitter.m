function emitter_id=addEmitter(obj,clips,varargin)
    % Add an emitter for one or more audio clips with rendering properties
    % and return the emitter ID.
    %
    % ADDEMITTER(OBJ,CLIPS) adds an emitter with the given clip IDs.
    %
    ...
    allowed_nodes=(1:numel(obj.nodes))-1;
    ips=inputParser;
    ips.addParameter('addToNode',[],@isnumeric);
    ips.addParameter('addToScene',false,@islogical);
    ips.addParameter('weights',[],@isnumeric);
    ips.addParameter('distanceModel',missing,@(x)gltf.GLTF.validateString(x,distanceModelValues));
    ...
    if(~isempty(clips))
        ...
        obj.addExtension("MSFT_audio_emitter");
        if(~isprop(obj,'extensions'))
            obj.addprop('extensions');
        end
        if(isfield(obj.extensions,'MSFT_audio_emitter') && isfield(obj.extensions.MSFT_audio_emitter,'emitters') && ~isempty(obj.extensions.MSFT_audio_emitter.emitters))
            emitter_id=numel([obj.extensions.MSFT_audio_emitter.emitters{:}]);
            obj.extensions.MSFT_audio_emitter.emitters=[obj.extensions.MSFT_audio_emitter.emitters{:} emitter_struct];
        else
            emitter_id=0;
            obj.extensions.MSFT_audio_emitter.emitters={emitter_struct};
        end
        if(~isempty(addToNode))
            for i=1:numel(addToNode)
                node_idx=addToNode(i)+1;
                if(isfield(obj.nodes{node_idx},'extensions') && isfield(obj.nodes{node_idx}.extensions,'MSFT_audio_emitter') && isfield(obj.nodes{node_idx}.extensions.MSFT_audio_emitter,'emitters'))
                    obj.nodes{node_idx}.extensions.MSFT_audio_emitter.emitters=[obj.nodes{node_idx}.extensions.MSFT_audio_emitter.emitters{:} emitter_id];
                else
                    obj.nodes{node_idx}.extensions.MSFT_audio_emitter.emitters=gltf.GLTF.toCells(emitter_id);
                end
            end
        elseif(addToScene)
            if(isfield(obj.scenes{1},'extensions') && isfield(obj.scenes{1}.extensions,'MSFT_audio_emitter') && isfield(obj.scenes{1}.extensions.MSFT_audio_emitter,'emitters'))
                obj.scenes{1}.extensions.MSFT_audio_emitter.emitters=[obj.scenes{1}.extensions.MSFT_audio_emitter.emitters{:} emitter_id];
            else
                obj.scenes{1}.extensions.MSFT_audio_emitter.emitters=gltf.GLTF.toCells(emitter_id);
            end
        end
    end
end
