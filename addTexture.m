function texture_idx=addTexture(gltf,image,varargin)
    ...
    if(ismissing(name))
        gltf.textures=[gltf.textures {struct('sampler',sampler_idx,'source',image_idx)}];
    else
        gltf.textures=[gltf.textures {struct('sampler',sampler_idx,'source',image_idx,'name',name)}];
    end
end
