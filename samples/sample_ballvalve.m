...
tube_mesh=obj.addMesh(V_tube,'indices',F_tube,'material',mat(1),'normals',true);
...
% Add the ball mesh.
% ball_mesh=obj.addMesh(V_ball,'indices',F_ball,'material',mat(2),'normals',true);                                     % Uncomment for non-skeletal animation
ball_mesh=obj.addMesh(V_ball,'indices',F_ball,'material',mat(2),'normals',true,'WEIGHTS',w_ball,'JOINTS',j_ball);  % Comment for non-skeletal animation
...
% obj.addPrimitiveToMesh(ball_mesh,V_shaft,'indices',F_shaft,'material',mat(3),'normals',true);                                            % Uncomment for non-skeletal animation
% obj.addPrimitiveToMesh(ball_mesh,V_handle,'indices',F_handle,'material',mat(4),'normals',true);                                          % Uncomment for non-skeletal animation
obj.addPrimitiveToMesh(ball_mesh,V_shaft,'indices',F_shaft,'material',mat(3),'normals',true,'WEIGHTS',w_shaft,'JOINTS',j_shaft);       % Comment for non-skeletal animation
obj.addPrimitiveToMesh(ball_mesh,V_handle,'indices',F_handle,'material',mat(4),'normals',true,'WEIGHTS',w_handle,'JOINTS',j_handle);   % Comment for non-skeletal animation
% Add the extra node and the skin (with the inverse bind matrix).
base_node=obj.addNode();                 % Comment for non-skeletal animation
skin_idx=obj.addSkin(base_node,'inverseBindMatrices',ibm); % Comment for non-skeletal animation
% Now add the node containing this compound mesh.
% ball_node=obj.addNode('mesh',ball_mesh);                                     % Uncomment for non-skeletal animation
ball_node=obj.addNode('mesh',ball_mesh,'skin',skin_idx);  % Comment for non-skeletal animation
...
% channel=obj.addAnimationChannel(0,ball_node,"rotation"); % Uncomment for non-skeletal animation
channel=obj.addAnimationChannel(0,base_node,"rotation");   % Comment for non-skeletal animation
