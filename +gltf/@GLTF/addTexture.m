function texture_idx=addTexture(obj,image,varargin)
    % Add a texture.
    %
    % ADDTEXTURE(OBJ,IMAGE) adds the IMAGE as a new texture, normal map,
    ...
    ips.addParameter('magFilter',missing,@(x)gltf.GLTF.validateString(x,filter_str_values));
    ips.addParameter('minFilter',missing,@(x)gltf.GLTF.validateString(x,filter_str_values));
    ips.addParameter('wrapS',missing,@(x)gltf.GLTF.validateString(x,wrap_str_values));
    ips.addParameter('wrapT',missing,@(x)gltf.GLTF.validateString(x,wrap_str_values));
    ...
    if(isnan(sampler_idx))
        sampler_idx=obj.addTextureSampler('magFilter',magFilter,'minFilter',minFilter,'wrapS',wrapS,'wrapT',wrapT);
    end
    if(isstring(image))
        image_idx=obj.addImage(image,'embedTexture',embedTexture);
    else
        image_idx=image;
    end
    if(~isprop(obj,'textures'))
        obj.addprop('textures');
    end
    texture_idx=numel(obj.textures);
    if(ismissing(name))
    ...
    if(~ismissing(webpImage))
        obj.addExtension("EXT_texture_webp");
        webp_idx=obj.addImage(webpImage,'embedTexture',embedTexture);
        EXT_texture_webp_struct=struct('source',webp_idx);
        texturestruct.extensions.EXT_texture_webp=EXT_texture_webp_struct;
    end
    obj.textures=[obj.textures {texturestruct}];
end
