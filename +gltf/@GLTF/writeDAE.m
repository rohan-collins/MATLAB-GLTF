function writeDAE(obj,filename,varargin)
    % Write a COLLADA file.
    %
    % WRITEDAE(OBJ,FILENAME) writes GLTF to a COLLADA file specified by
    % filename.
    %
    ...
    ips=inputParser;
    ips.addParameter('up_axis',"Y_UP",@(x)gltf.GLTF.validateString(x,up_axis_str_values));
    ...
    instance_visual_scene.setAttribute("url","#scene_"+string(obj.scene+1));
    ...
    node_list=cell(0,1);
    for scene_id=1:numel(obj.scenes)
        visual_scene=documentNode.createElement("visual_scene");
        visual_scene.setAttribute("id","scene_"+string(scene_id));
        [pred,isMesh]=obj.nodeTree();
        rootAndMesh=and(pred==0,isMesh);
        for node_id=find(rootAndMesh)'
            [node,library_geometries,library_controllers,node_list]=obj.getNode(documentNode,library_geometries,library_controllers,node_id,node_list,normals,tangents,binormals);
    ...
    if(isprop(obj,'images'))
        ...
        for image_id=1:numel(obj.images)
            ...
            init_from.appendChild(documentNode.createTextNode(obj.images{image_id}.uri));
    ...
    if(isprop(obj,'materials'))
        ...
        for material_id=1:numel(obj.materials)
            ...
            if(isfield(obj.materials{material_id}.pbrMetallicRoughness,'baseColorTexture'))
                texture_id=obj.materials{material_id}.pbrMetallicRoughness.baseColorTexture.index+1;
                image_id=obj.textures{texture_id}.source+1;
                ...
                color.appendChild(documentNode.createTextNode(strip(sprintf(gltf.GLTF.formatSpec_float(),obj.materials{material_id}.pbrMetallicRoughness.baseColorFactor))));
                diffuse.appendChild(color);
            end

            lambert.appendChild(diffuse);
            if(isfield(obj.materials{material_id}.pbrMetallicRoughness,'baseColorFactor'))
                if(obj.materials{material_id}.pbrMetallicRoughness.baseColorFactor(4)<1)
                    transparency=documentNode.createElement("transparency");
                    float=documentNode.createElement("float");
                    float.appendChild(documentNode.createTextNode(strip(sprintf(gltf.GLTF.formatSpec_float(),obj.materials{material_id}.pbrMetallicRoughness.baseColorFactor(4)))));
    ...
    if(isprop(obj,'animations'))
        ...
        for animation_id=1:numel(obj.animations)
            ...
            for channel_id=1:numel(obj.animations{animation_id}.channels)
                ...
                if(obj.animations{animation_id}.channels{channel_id}.target.path=="translation")
                    sampler_id=obj.animations{animation_id}.channels{channel_id}.sampler+1;
                    input_data=obj.getAccessor(obj.animations{animation_id}.samplers{sampler_id}.input);
                    output_data=obj.getAccessor(obj.animations{animation_id}.samplers{sampler_id}.output);
                    if(isfield(obj.animations{animation_id}.samplers{sampler_id},'interpolation'))
                        interp_data=obj.getAccessor(obj.animations{1}.samplers{sampler_id}.interpolation);
                    else
                        interp_data="LINEAR";
                    end
                    source=documentNode.createElement("source");
                    ...
                    float_array.appendChild(documentNode.createTextNode(strip(sprintf(gltf.GLTF.formatSpec_float(),input_data))));
                    ...
                    float_array.appendChild(documentNode.createTextNode(strip(sprintf(gltf.GLTF.formatSpec_float(),output_data'))));
                    source.appendChild(float_array);
                    technique_common=documentNode.createElement("technique_common");
                    accessor=documentNode.createElement("accessor");
                    ...
                    subanimation.appendChild(source);
                    target="translate";
                    target_node_id=obj.animations{animation_id}.channels{channel_id}.target.node+1;
                    if(isempty(node_list{target_node_id}.getElementsByTagName("translate").item(0)))
                        ...
                        translate.appendChild(documentNode.createTextNode(strip(sprintf(gltf.GLTF.formatSpec_float(),[0 0 0]))));
                    ...
                    channel.setAttribute("target","node_"+string(obj.animations{animation_id}.channels{channel_id}.target.node+1)+"/"+target);
                    subanimation.appendChild(channel);
                elseif(obj.animations{animation_id}.channels{channel_id}.target.path=="rotation")
                    sampler_id=obj.animations{animation_id}.channels{channel_id}.sampler+1;
                    input_data=obj.getAccessor(obj.animations{animation_id}.samplers{sampler_id}.input);
                    output_data=obj.getAccessor(obj.animations{animation_id}.samplers{sampler_id}.output);
                    if(isfield(obj.animations{animation_id}.samplers{sampler_id},'interpolation'))
                        interp_data=obj.animations{1}.samplers{sampler_id}.interpolation;
                    else
                        interp_data="LINEAR";
                    end
                    source=documentNode.createElement("source");
                    ...
                    source.appendChild(technique_common);
                    subanimation.appendChild(source);
                    output_data=gltf.GLTF.Q2AxisAngle(output_data);
                    source=documentNode.createElement("source");
                    source.setAttribute("id","animation_"+string(animation_id)+"_"+string(channel_id)+"_output");
                    float_array=documentNode.createElement("float_array");
                    float_array.setAttribute("id","animation_"+string(animation_id)+"_"+string(channel_id)+"_output-array");
                    ...
                    if(isempty(node_list{target_node_id}.getElementsByTagName("rotate").item(0)))
                        ...
                        rotate.appendChild(documentNode.createTextNode(strip(sprintf(gltf.GLTF.formatSpec_float(),[0 1 0 0]))));
                    ...
                    channel.setAttribute("target","node_"+string(obj.animations{animation_id}.channels{channel_id}.target.node+1)+"/"+target);
                    subanimation.appendChild(channel);
                elseif(obj.animations{animation_id}.channels{channel_id}.target.path=="scale")
                    sampler_id=obj.animations{animation_id}.channels{channel_id}.sampler+1;
                    input_data=obj.getAccessor(obj.animations{animation_id}.samplers{sampler_id}.input);
                    output_data=obj.getAccessor(obj.animations{animation_id}.samplers{sampler_id}.output);
                    if(isfield(obj.animations{animation_id}.samplers{sampler_id},'interpolation'))
                    ...
                    if(isempty(node_list{target_node_id}.getElementsByTagName("scale").item(0)))
                        ...
                        scale.appendChild(documentNode.createTextNode(strip(sprintf(gltf.GLTF.formatSpec_float(),[1 1 1]))));
                    ...
                    channel.setAttribute("target","node_"+string(obj.animations{animation_id}.channels{channel_id}.target.node+1)+"/"+target);
                    subanimation.appendChild(channel);
                elseif(obj.animations{animation_id}.channels{channel_id}.target.path=="weights")
                    mesh_idx=obj.nodes{obj.animations{animation_id}.channels{channel_id}.target.node+1}.mesh+1;
                    sampler_id=obj.animations{animation_id}.channels{channel_id}.sampler+1;
                    input_data=obj.getAccessor(obj.animations{animation_id}.samplers{sampler_id}.input);
                    output_data=obj.getAccessor(obj.animations{animation_id}.samplers{sampler_id}.output);
                    ...
                    for primitive_id=1:numel(obj.meshes{obj.nodes{obj.animations{animation_id}.channels{channel_id}.target.node+1}.mesh+1}.primitives)
                        ...
                        for target_id=1:numel(obj.meshes{obj.nodes{obj.animations{animation_id}.channels{channel_id}.target.node+1}.mesh+1}.primitives{1}.targets)
                            ...
                            float_array.appendChild(documentNode.createTextNode(strip(sprintf(gltf.GLTF.formatSpec_float(),output_data(:,target_id)'))));
                            source.appendChild(float_array);
                            technique_common=documentNode.createElement("technique_common");
                            accessor=documentNode.createElement("accessor");
                            accessor.setAttribute("count",string(size(output_data,1)));
