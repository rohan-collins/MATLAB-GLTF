...
edge_idx=obj.addMaterial('baseColorFactor',s(1).EdgeColor);
% Add the material for the text colour.
text_idx=obj.addMaterial('baseColorFactor',ones(1,3));
% Read fonts
fontFile=gltf.utilities.readFontFile("ARIALUNI_1.svg");
for i=1:numel(x)
    ...
    face_idx=obj.addMaterial('baseColorFactor',C(i,:));
    % Add the mesh with faces, vertices, and the face material.
    mesh_idx=obj.addMesh(V,'indices',F,'material',face_idx);
    ...
    % the mesh primitive.
    obj.meshes{mesh_idx+1}.primitives=repmat(obj.meshes{mesh_idx+1}.primitives,1,2);
    ...
    edge_indices=obj.addBinaryData(Etemp,"UNSIGNED_INT","SCALAR",false,"ELEMENT_ARRAY_BUFFER");
    % We set the indices accessor for the second primitive to be the one we
    % just added.
    obj.meshes{mesh_idx+1}.primitives{2}.indices=edge_indices;
    % And add the mode to be "LINES", enumerated as 1 in GLTF or OpenGL.
    obj.meshes{mesh_idx+1}.primitives{2}.mode=1;
    % Change the material to the edge material.
    obj.meshes{mesh_idx+1}.primitives{2}.material=edge_idx;
    ...
    [Ft,Vt]=gltf.utilities.text2FV(string(s(p3).String),fontFile);
    ...
    obj.addNode('mesh',obj.addMesh(Vt,'indices',Ft,'material',text_idx),'rotation',q);
end
% Write the GLTF file.
obj.writeGLTF("pie3.gltf");
