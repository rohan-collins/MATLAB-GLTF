function writePLY(obj,filename,varargin)
    % Write a Stanford PLY file.
    %
    % WRITEPLY(OBJ,FILENAME) writes GLTF to a Wavefront OBJ file specified
    % by filename.
    %
    % WRITEPLY(OBJ,FILENAME,'nodes',NODES) writes the specified NODES in
    % the GLTF to a Wavefront OBJ file specified by filename. Each of the
    % specified nodes should be a mesh node.
    %
    % WRITEPLY(OBJ,FILENAME,'meshes',MESHES) writes the specified MESHES
    ...
    if(~or(all(nonormals),all(~nonormals)))
        ...
        for i=1:numel(I)
            if(edges(I(i)))
                ...
                [N,F,idx]=gltf.GLTF.vertexNormals(nodeF{I(i)},nodeV{I(i)});
                nodeF{I(i)}=F;
