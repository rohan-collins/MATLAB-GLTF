function data=getAccessor(obj,accessor_idx)
    % Get data referenced by an accessor.
    %
    % GETACCESSOR(OBJ,ACCESSOR_IDX) returns the data referenced by the
    ...
    accessor=obj.accessors{accessor_idx+1};
    bufferView=obj.bufferViews{accessor.bufferView+1};
    buffer=obj.buffers{bufferView.buffer+1};
