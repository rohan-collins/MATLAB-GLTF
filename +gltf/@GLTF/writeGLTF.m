function writeGLTF(obj,filename,varargin)
    % Write a GLTF file.
    %
    % WRITEGLTF(OBJ,FILENAME) writes GLTF to a GLTF-file specified by
    % FILENAME.
    %
    ...
    if(isfield(st,"images"))
        for i=1:numel(st.images)
            if(~startsWith(st.images{i}.uri,"data:"))
                [~,relative2]=gltf.GLTF.getRelativePath(filename,st.images{i}.uri);
    ...
    if(isfield(st,'extensions') && isfield(st.extensions,'MSFT_audio_emitter') && isfield(st.extensions.MSFT_audio_emitter,'clips'))
        for i=1:numel(st.extensions.MSFT_audio_emitter.clips)
            if(~startsWith(st.extensions.MSFT_audio_emitter.clips{i}.uri,"data:"))
                [~,relative2]=gltf.GLTF.getRelativePath(filename,st.extensions.MSFT_audio_emitter.clips{i}.uri);
    ...
    if(ismissing(bufferFile))
        for i=1:numel(obj.buffers)
            if(~isstruct(st.buffers{i}))
                st.buffers{i}=struct('uri',['data:application/octet-stream;base64,' matlab.net.base64encode(obj.buffers{i})],'byteLength',uint32(numel(obj.buffers{i})));
            end
        end
    else
        if(numel(obj.buffers)>1)
            ...
            st.buffers{1}=struct('uri',relative2,'byteLength',uint32(numel(obj.buffers{1})));
        end
    end
